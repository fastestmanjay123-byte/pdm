...
def decimate_for_chart(df, ts_col, max_points=5000):
    """Keep at most ~max_points rows (for fast web plotting)."""
    n = len(df)
    if n <= max_points:
        return df
...
with left:
    ...
    # shade failure windows if provided
    if shade_fail and label_col and "label" in df.columns and df["label"].sum() > 0:
        # compute contiguous label==1 spans
        spans = []
        ...
        for i in range(len(lab)):
            ...
            if lab[i]==0 and idx_start is not None:
                spans.append((df["timestamp"].iloc[idx_start], df["timestamp"].iloc[i-1]))
                idx_start = None
        ...
        if spans:
            ...
            boxes = alt.Chart(regions).mark_rect(opacity=0.15).encode(
                x="start:T", x2="end:T"
            )
            layer = boxes + layer
...
if show_confusion and label_col and "label" in df.columns:
    ...
    heat = alt.Chart(cm_df).mark_rect().encode(
        x="Predicted:N", y="Actual:N", color="Count:Q"
    )
    text = alt.Chart(cm_df).mark_text(fontWeight="bold").encode(
...
st.caption("Tip: Use the chart menu (upper-right) to download images for your manuscript.")
